...
```{r}
ts_electricity_price_full <- ts(electricity_price_processed[1:nobs, "All.sectors"], start = c(year(electricity_price_processed$Month[1]), month(electricity_price_processed$Month[1])), frequency = 12)

autoplot(ts_electricity_price_full) + autolayer(MEAN_seas, series = "Mean", PI = FALSE) + autolayer(SNAIVE_seas, series = "Seasonal Naive", PI = FALSE) + autolayer(SARIMA_for, series = "SARIMA", PI = FALSE)
