...
```{r}
wooclap_linear_trend_model <- lm(inflow_data[,8+1]~t)
summary(wooclap_linear_trend_model)

#Store reg coefficient
wooclap_beta1 <- as.numeric(wooclap_linear_trend_model$coefficients[2]) #slope
...
```{r}
#Fit a linear trend to TS of iHP
wooclap_linear_trend_model <- lm(inflow_data[,3+1]~t)

#Store reg coefficient
wooclap_beta0 <- as.numeric(wooclap_linear_trend_model$coefficients[1]) #intercept
wooclap_beta1 <- as.numeric(wooclap_linear_trend_model$coefficients[2]) #slope

#remove the trend from series
wooclap_detrend_inflow_data <- inflow_data[,3 + 1] - (wooclap_beta0 + (wooclap_beta1 * t))
class(wooclap_detrend_inflow_data)

#Transform to time series
wooclap_ts_detrend_inflow <- ts(wooclap_detrend_inflow_data, frequency = 12, start = c(1931, 1))

wooclap_dummies <- seasonaldummy(wooclap_ts_detrend_inflow)

wooclap_seas_means_model <- lm(wooclap_detrend_inflow_data~wooclap_dummies)

summary(wooclap_seas_means_model)
```
